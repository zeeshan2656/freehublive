...
class FHVideoAdManager {
  ...
  async trackImpression(adId) {
    try {
      const url = `${this.baseUrl}/api/ads.php?action=track_impression&id=${adId}&video_id=${this.videoId}`;
  ...
  async trackClick(adId) {
    try {
      const url = `${this.baseUrl}/api/ads.php?action=track_click&id=${adId}&video_id=${this.videoId}`;
      await fetch(url, { method: 'POST' });
    } catch (e) {}
  }
